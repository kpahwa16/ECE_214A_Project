function output=NSSP(signal,fs,IS)

%  This is the main MATLAB function of implementation of algorithm presented in
...
%  This code is developed with help of Matlab code of spectral subtraction
...
W=fix(.012*fs); %Window length is 25 ms
nfft=W;
...
lowFreqSample=(nfft/fs)*50;
range=(1):(1+lowFreqSample);

for i=1:numberOfFrames
    [~, SpeechFlag, NoiseCounter, ~]=vad(Y(:,i).^(1/Gamma),N.^(1/Gamma),NoiseCounter); %Magnitude Spectrum Distance VAD
    
    if SpeechFlag==0
        N=(1-vn)*N+vn*Y(:,i); %Update and smooth noise
        NRM=max(NRM,YS(:,i)-N);%Update Maximum Noise Residue
        X(:,i)=Beta*Y(:,i);
    else
        Ykf=YS(:,i);
        Ykfmag=abs(Ykf);
        Wklowf=Ykfmag(range);% See Equation 5: First 10 samples are assumed as non speech segment
        lamdak=sum(Wklowf)./sum(abs(N(range))); % See equation 6
        D=(Ykfmag-mu*lamdak*abs(N)).*(Ykf./Ykfmag);
...
Step1out=OverlapAdd2(X.^(1/Gamma),YPhase,W,SP*W);

%% Second Phase of the algorithm
output = SNRdependentPSC( Step1out, fs, 32, 32/4, 'Griffin & Lim', 1 );
